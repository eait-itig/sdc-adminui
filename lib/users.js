            ...
            func: function (role, callback) {
                ...
                vasync.forEachParallel({
                    ...
                    func: function (member, cb) {
                        ...
                        var subUuids = member.match(
                            /* JSSTYLED */
                            /^uuid=([a-f0-9-]+), uuid=([a-f0-9-]+), ou=/);
                        /* JSSTYLED */
                        var acctUuid = member.match(/^uuid=([a-f0-9-]+), ou=/);
                        if (subUuids) {
                            req.ufds.getUser(subUuids[1], subUuids[2],
                                function (error) {
                            ...
                            req.ufds.getUser(acctUuid[1], function (error) {
                                if (error) {
                                    if (error.body && error.body.code === 'ResourceNotFound') {
                                        memberExists = false;
                                    } else {
